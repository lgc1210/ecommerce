...
- **GHN (Giao Hàng Nhanh)**: tra cứu tỉnh/huyện/xã, tính phí vận chuyển thực tế theo kích thước/khối lượng từng SKU, tạo đơn, nhận webhook cập nhật trạng thái vận chuyển (`ghnStatus`, tách biệt với `orderStatus` nội bộ). Tạo vận đơn tự retry tại chỗ (tối đa 3 lần) khi gặp lỗi tạm thời phía GHN (timeout nội bộ, quá tải); nếu vẫn thất bại, job nền định kỳ (`GHN_SHIPMENT_RETRY_CRON`) quét các đơn đã thanh toán "completed" nhưng chưa có vận đơn để tạo lại — đơn hàng không bị kẹt vì 1 lần gọi GHN lỗi thoáng qua.
- **Cổng thanh toán**: VNPay và ZaloPay đã có gateway triển khai đầy đủ (`features/payments/gateways/`); MoMo/Stripe/PayPal đã có trong enum `PaymentMethod` và biến môi trường mẫu nhưng gateway MoMo chưa thấy code triển khai trong `gateways/` (chỉ VNPay + ZaloPay). Đơn đã bị hủy (`orderStatus: cancelled`) bị khoá hẳn khỏi mọi thao tác thanh toán (tạo giao dịch mới, xác nhận, hoàn tất qua IPN) dù `paymentStatus` đang ở trạng thái nào.
...
- **Cron job nội bộ**: (1) tự động hủy đơn "pending" thanh toán online quá hạn (`PENDING_ORDER_TTL_HOURS`, mặc định 24h), hoàn lại tồn kho + lượt dùng coupon; không áp dụng cho đơn COD — cùng lịch chạy này cũng dọn luôn các `checkout_idempotency_keys` cũ quá 24h (gate chống double-submit của luồng "Mua ngay"); (2) retry tạo vận đơn GHN cho các đơn đã thanh toán "completed" nhưng vẫn chưa có vận đơn (`GHN_SHIPMENT_RETRY_CRON`, mặc định mỗi 15 phút), cảnh báo admin nếu sau khi retry vẫn còn đơn tồn đọng.

### 2.7. Biến môi trường

Xem đầy đủ và có chú thích chi tiết bằng tiếng Việt tại `backend/.env.example`, gồm các nhóm: Core (`PORT`, `CLIENT_URL`), `DATABASE_URL` (MySQL), JWT secrets, SMTP, Google/Facebook OAuth, VNPay/MoMo/ZaloPay, GHN, cấu hình job dọn đơn quá hạn (`PENDING_ORDER_CLEANUP_CRON`, `PENDING_ORDER_TTL_HOURS`) và job retry vận đơn GHN (`GHN_SHIPMENT_RETRY_CRON`, có giá trị mặc định nên không bắt buộc khai báo).
...
- **Tích hợp vận chuyển & thanh toán thực tế cho thị trường Việt Nam**: GHN tính phí ship theo kích thước/khối lượng từng biến thể sản phẩm (tự retry khi GHN lỗi tạm thời, có job nền dọn các đơn còn sót vận đơn), VNPay & ZaloPay, cơ chế tự hủy đơn "pending" quá hạn. Đơn đã hủy bị khoá hẳn khỏi mọi thao tác thanh toán; khách có thể tự đổi phương thức thanh toán (COD ⇄ online) hoặc "đặt lại" đơn đã hủy ngay trong trang tài khoản.
