...
const FormCheckbox = forwardRef<HTMLInputElement, FormCheckboxProps>(({ label, error, wrapperClassName = "", id, className = "", ...rest }, ref) => {
	const generatedId = useId();
	const fieldId = id ?? generatedId;

	return (
		<div className={wrapperClassName}>
			<label htmlFor={fieldId} className='flex items-start gap-2 text-sm text-ink/80'>
				<input
					id={fieldId}
					ref={ref}
					type='checkbox'
