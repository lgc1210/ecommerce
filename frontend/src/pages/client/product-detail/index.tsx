...
const ProductDetailPage = () => {
	...
	const [activeTab, setActiveTab] = useState<(typeof tabs)[number]["id"]>(
		hash.startsWith("#review-") ? "reviews" : "description",
	);
	...
	const hasSelectedAllAttributes =
		attributes.length > 0 && attributes.every((attribute) => Boolean(selected[attribute]));
			...
			<div className='mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8'>
				<div className='grid gap-10 lg:grid-cols-2'>
					...
					<div>
						...
						<h1
							className='mt-3 text-3xl font-extrabold tracking-tight text-ink'
							style={{ viewTransitionName: `product-title-${product.name}` }}>
							{product.name}
						</h1>
						...
						<div className='mt-4 flex items-center gap-3'>
							...
							<span className='text-3xl font-bold text-primary-dark'>
								{!selectedSku && priceRange.min !== priceRange.max && (
									<span className='mr-1 text-lg font-semibold text-muted'>Từ</span>
								)}
						...
						<div className='mt-8 flex flex-wrap items-center gap-2 select-none'>
							<QuantityStepper
								value={quantity}
								max={selectedSku?.stockQuantity ?? 1}
							...
							<div className='flex flex-wrap gap-2'>
								{isAuthenticated && (
									<Button
										type='button'
										variant='outline'
										disabled={!inStock || isMutating || !hasSelectedAllAttributes}
					...
					<div className='max-w-3xl py-8 text-sm leading-relaxed text-ink/80'>
						...
						{activeTab === "specs" && (
							<ProductSpecsTab
								...
								sku={selectedSku?.sku}
								inStock={inStock}
								isUnavailableCombination={isUnavailableCombination}
							/>
						)}
