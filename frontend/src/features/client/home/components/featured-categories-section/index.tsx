...
import FeaturedCategoriesSectionSkeleton from "./skeleton";
...
const getCategoryImage = (name: string) =>
	`https://placehold.co/200x200/f3ede4/1c1815?font=montserrat&text=${encodeURIComponent(name)}`;
...
const FeaturedCategoriesSection = () => {
	const { data: featuredCategoriesData, isLoading: isFeaturedCategoriesLoading } =
		useFeaturedCategoriesQuery(FEATURED_CATEGORIES_LIMIT);
	...
	const { data: allCategoriesData, isLoading: isAllCategoriesLoading } = useCategoriesQuery(
	...
	return (
		<section className='mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8'>
			...
			{isLoading ? (
				<FeaturedCategoriesSectionSkeleton />
