						...
						{isLoading ? (
							...
							orders.map((order) => (
								<tr key={order.id} className='border-b border-border last:border-0'>
									<td className='py-2.5 pr-3'>
										<Link
											to={`${paths.admin.order}?search=${encodeURIComponent(order.orderNumber)}`}
									...
									<td className='py-2.5 pr-3'>
										{order.payment ? <PaymentStatusBadge status={order.payment.paymentStatus} /> : "—"}
									</td>
									<td className='py-2.5 pr-3 font-medium text-ink'>{formatCurrency(Number(order.totalAmount))}</td>
									<td className='py-2.5 pr-3'>
										<OrderStatusBadge status={order.orderStatus} />
									</td>
									<td className='py-2.5 text-ink/70'>{formatOrderDate(order.createdAt)}</td>
								</tr>
							))
						)}
					</tbody>
