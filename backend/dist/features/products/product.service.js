...
import { DEFAULT_SKU_HEIGHT_CM, DEFAULT_SKU_LENGTH_CM, DEFAULT_SKU_WEIGHT_GRAM, DEFAULT_SKU_WIDTH_CM, productSort } from "./product.constant.js";
...
class ProductService {
    ...
    async createProduct(data) {
        ...
        const providedSkuCodes = (data.skus ?? []).map((s) => s.sku).filter((code) => !!code);
        ...
        if (data.skus && data.skus.length > 0) {
            ...
            const reservedCodes = new Set(providedSkuCodes);
            const resolvedSkus = [];
    ...
    async createSku(productId, data) {
        ...
        return prisma.productSku.create({
            data: {
                ...
                variationDetails: data.variationDetails,
    ...
    async updateSku(productId, skuId, data) {
        ...
        if (data.variationDetails !== undefined)
            updateData.variationDetails = data.variationDetails;
        if (data.weightGram !== undefined)
            updateData.weightGram = data.weightGram;
        if (data.lengthCm !== undefined)
            updateData.lengthCm = data.lengthCm;
        if (data.widthCm !== undefined)
            updateData.widthCm = data.widthCm;
    ...
    async updateSkuStock(productId, skuId, stockQuantity) {
    ...
    async addSkuImage(productId, skuId, data) {
        await this.assertSkuBelongsToProduct(productId, skuId);
    ...
    async updateSkuImage(productId, skuId, imageId, data) {
    ...
    async deleteSkuImage(productId, skuId, imageId) {
     ...
     */
    async findRelatedProducts(productId, categoryId) {
        if (!categoryId)
            return [];
     ...
     */
    async resolvePriceSortedProducts(where, sort, skip, take) {
        ...
        const minPriceById = new Map(priceGroups.filter((group) => group.productId !== null).map((group) => [group.productId, Number(group._min.price)]));
    ...
    async assertSkuBelongsToProduct(productId, skuId) {
    ...
    async assertImageBelongsToSku(skuId, imageId) {
     ...
     */
    async syncProductThumbnail(productId, tx = prisma) {
    ...
    async assertSkuCodesAvailable(skuCodes) {
    ...
    async resolveUniqueSlug(source) {
     ...
     */
    async resolveUniqueSkuCode(baseCode, reservedCodes) {
