...
class CartService {
    ...
    async mergeLocalCartToDb(userId, pendingCartItems) {
        const skippedItems = [];
        ...
        const mergedLocalItems = new Map();
        ...
        for (const [skuId, localQty] of mergedLocalItems) {
            ...
            if (existing) {
                ...
                if (newQuantity !== existing.quantity) {
                    operations.push(prisma.cartItem.update({
                ...
                operations.push(prisma.cartItem.create({
                    data: {
                        cartId: cart.id,
                        productSkuId: skuId,
    ...
    async assertSkuAvailable(productSkuId) {
    ...
    async getOwnedItemOrThrow(userId, itemId) {
