...
const writeRows = async (res, headers, readBatch) => {
...
const buildReader = (resource, query) => {
    switch (resource) {
        case "users": {
            const where = {
        ...
        case "products": {
            ...
            return {
                ...
                readBatch: async (cursor) => {
                    ...
                    return products.flatMap((product) => (product.skus.length ? product.skus : [null]).map((sku) => ({
                        id: product.id,
                        name: product.name,
                        slug: product.slug,
        ...
        case "orders": {
            ...
            return {
                ...
                headers,
                readBatch: async (cursor) => {
                    const orders = await prisma.order.findMany({ where: where, take: BATCH_SIZE, ...(cursor ? { skip: 1, cursor: { id: cursor } } : {}), orderBy: { id: "asc" }, include: { user: { select: { name: true, email: true } }, payment: { select: { paymentStatus: true, paymentMethod: true } } } });
        ...
        case "payments": {
            ...
            return {
                ...
                headers,
                readBatch: async (cursor) => {
                    const payments = await prisma.payment.findMany({ where: where, take: BATCH_SIZE, ...(cursor ? { skip: 1, cursor: { id: cursor } } : {}), orderBy: { id: "asc" }, include: { order: { select: { orderNumber: true, user: { select: { name: true, email: true } } } } } });
...
export const exportResource = async (resource, query, res) => {
