 ...
 */
function sampleShippingFee(subtotal) {
...
export const orderSeed = async () => {
    ...
    if (customers.length === 0 || skus.length === 0)
        return;
    const sampleStatuses = ["pending", "processing", "shipped", "delivered", "cancelled"];
    const samplePaymentMethods = ["cod", "vnpay", "momo"];
    let skuCursor = 0;
            ...
            data: {
                ...
                items: {
                    create: items.map((item) => ({
                        ...
                        variationSnapshot: item.sku.variationDetails,
                    })),
                },
                payment: {
                    create: {
...
//# sourceMappingURL=order.seed.js.map