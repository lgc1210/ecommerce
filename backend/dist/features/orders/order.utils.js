...
export function mapGhnStatusToOrderStatus(ghnStatus) {
    if (GHN_DELIVERED_STATUSES.has(ghnStatus))
        return OrderStatus.delivered;
    if (GHN_CANCELLED_STATUSES.has(ghnStatus))
        return OrderStatus.cancelled;
    if (GHN_SHIPPED_STATUSES.has(ghnStatus))
        return OrderStatus.shipped;
    if (GHN_PROCESSING_STATUSES.has(ghnStatus))
 ...
 */
export function computeCartPackage(items) {
    let weightGram = 0;
    let lengthCm = 0;
    let widthCm = 0;
    let heightCm = 0;
