...
function isWithinReviewWindow(deliveredAt) {
    if (!deliveredAt)
        return false;
...
const reviewWithProductAndUserInclude = {
    ...
    product: { select: { id: true, name: true, slug: true } },
    reply: { include: { repliedByUser: { select: { id: true, name: true } } } },
    moderationLogs: { orderBy: { createdAt: "desc" }, take: 1, include: { actionByUser: { select: { id: true, name: true } } } },
};
class ReviewService {
    // ==========================================
    // Public
    ...
    async listReviewableOrderItems(userId) {
    ...
    async listMyReviews(userId, params) {
        const where = { userId };
        const { page, limit, skip } = parsePagination(params);
    ...
    async updateReview(userId, reviewId, data) {
        ...
        const updateData = { editCount: { increment: 1 }, lastEditedAt: new Date() };
    ...
    async listReviewsAdmin(params) {
        ...
        if (params.isVisible !== undefined)
            where.isVisible = params.isVisible === "true";
        if (params.search)
            where.comment = { contains: params.search };
        const { page, limit, skip } = parsePagination(params);
        const [reviews, total] = await Promise.all([
     ...
     */
    async hideReview(actionByUserId, reviewId, data) {
        const review = await this.getReviewOrThrow(reviewId);
        if (!review.isVisible) {
    ...
    async unhideReview(actionByUserId, reviewId, data) {
        const review = await this.getReviewOrThrow(reviewId);
        if (review.isVisible) {
    ...
    // ==========================================
    async createReply(repliedBy, reviewId, data) {
    ...
    async updateReply(reviewId, data) {
    ...
    // Helpers
    // ==========================================
    async applyModeration(actionByUserId, reviewId, isHidden, reason) {
    ...
    async deleteReviewCascade(reviewId) {
    ...
    async getReviewOrThrow(reviewId) {
