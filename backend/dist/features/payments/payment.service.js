...
class PaymentService {
    ...
    // ==========================================
    async getOwnPayment(userId, orderId) {
     ...
     */
    async confirmOwnPayment(userId, orderId, transactionId) {
        const payment = await this.getPaymentByOrderOrThrow(orderId);
        if (payment.order.userId !== userId) {
            throw new Error("NotFound: Không tìm thấy thông tin thanh toán cho đơn hàng này.");
        }
     ...
     * failed -> pending).
     */
    async changeOwnPaymentMethod(userId, orderId, newMethod) {
        const payment = await this.getPaymentByOrderOrThrow(orderId);
        if (payment.order.userId !== userId) {
            throw new Error("NotFound: Không tìm thấy thông tin thanh toán cho đơn hàng này.");
        }
    ...
    async getGatewayPaymentContext(userId, orderId) {
        const payment = await this.getPaymentByOrderOrThrow(orderId);
        if (payment.order.userId !== userId) {
            throw new Error("NotFound: Không tìm thấy thông tin thanh toán cho đơn hàng này.");
        }
    ...
    async completeGatewayPayment(orderId, transactionId) {
        const payment = await this.getPaymentByOrderOrThrow(orderId);
    ...
    async failGatewayPayment(orderId, transactionId) {
        const payment = await this.getPaymentByOrderOrThrow(orderId);
    ...
    // Helpers
    // ==========================================
    async transitionStatus(payment, nextStatus, transactionId) {
        ...
        if (nextStatus === PaymentStatus.completed) {
            try {
                ...
                try {
                    await notificationService.notifyAdminSystemAlert("Lỗi tạo vận đơn GHN", `Đơn hàng ${payment.order.orderNumber} đã thanh toán thành công nhưng tạo vận đơn GHN thất bại. Cần kiểm tra và tạo vận đơn thủ công.`);
                }
                catch (notifyError) {
    ...
    async getPaymentByOrderOrThrow(orderId) {
    ...
    async getPaymentOrThrow(id, include) {
