...
export function generateWelcomeCouponCode() {
 ...
 */
export function checkCouponEmailOwnership(couponEmail, userEmail) {
    if (!couponEmail)
        return true;
    if (!userEmail)
        return false;
...
export function checkCouponUsability(coupon, now = new Date()) {
    if (!coupon.isActive) {
