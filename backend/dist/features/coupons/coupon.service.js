...
class CouponService {
    ...
    async listCoupons(params) {
        const where = {};
        if (params.search)
            where.code = { contains: normalizeCouponCode(params.search) };
    ...
    async updateCoupon(id, data) {
        ...
        if (data.maxDiscountValue !== undefined)
            updateData.maxDiscountValue = data.maxDiscountValue;
     ...
     */
    async validateCoupon(code, orderSubtotal, userEmail) {
     ...
     */
    async requestWelcomeCoupon(rawEmail) {
    ...
    async sendWelcomeCouponEmail(email, code) {
        await sendEmail({
            to: email,
