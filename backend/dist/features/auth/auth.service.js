...
class AuthService {
    ...
    async resendOtp({ email, type }) {
        ...
        await prisma.otp.updateMany({
            where: { target: email, type: type, status: OtpStatus.pending },
            data: { status: OtpStatus.expired },
        });
        await this.issueOtp(user.id, email, type);
    }
    // ==========================================
    // Đăng nhập
    // ==========================================
    async login({ email, password, cartItems: pendingCartItems }) {
    ...
    async loginWithGoogle({ accessToken: googleAccessToken, cartItems: pendingCartItems = [] }) {
    ...
    async loginWithFacebook({ accessToken, cartItems: pendingCartItems = [] }) {
        ...
        const email = facebookProfile.email.toLowerCase();
        const facebookUserId = facebookProfile.id;
    ...
    async refreshAccessToken(rawRefreshToken) {
        ...
        const freshPayload = { id: currentUser.id, email: currentUser.email, roleId: currentUser.roleId };
    ...
    async issueOtp(userId, target, type) {
    ...
    async validateAndConsumeOtp(target, otpCode, type) {
    ...
    async sendOtpEmail(target, otpCode, type) {
