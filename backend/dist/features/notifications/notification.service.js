...
import { buildOrderPlacedNotification, buildOrderStatusChangedNotification, buildPaymentCompletedNotification, buildPaymentFailedNotification, buildPaymentRefundedNotification, buildReviewRepliedNotification, buildAdminNewOrderContent, buildAdminLowStockContent, buildAdminPaymentFailedContent, buildAdminNewReviewContent, buildAdminSystemAlertContent, buildAdminNewContactContent, } from "./notification.utils.js";
...
class NotificationService {
    ...
    async notifyAdminNewOrder(orderId, orderNumber, totalAmount) {
    ...
    async notifyAdminLowStock(skuId, skuLabel, productId, productName, stockQuantity) {
    ...
    async notifyAdminPaymentFailed(orderId, orderNumber) {
    ...
    async notifyAdminNewReview(reviewId, productName, rating) {
     ...
     */
    async notifyAdminSystemAlert(title, message) {
    ...
    async notifyAdminNewContact(contactId, name, subject) {
    ...
    async broadcastToAllCustomers(data) {
        ...
        while (true) {
            ...
            if (customers.length === 0)
                break;
            await this.dispatch(customers.map((c) => ({
                userId: c.id,
                type: data.type,
                title: data.title,
            ...
            if (customers.length < BROADCAST_BATCH_SIZE)
                break;
        }
        return { sentCount };
    }
     ...
     */
    async notifyAdmins(content, permission) {
     ...
     */
    async getUserIdsWithPermission(resource, name) {
    ...
    async getOwnNotificationOrThrow(userId, id) {
