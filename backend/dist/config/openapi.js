...
function buildParameters(shape, location) {
    if (!shape)
        return [];
    const jsonSchema = toRequestJsonSchema(shape);
...
export function buildOpenApiDocument() {
    ...
    for (const route of routeManifest) {
        const bodyShape = route.schema?.shape?.body;
        const queryShape = route.schema?.shape?.query;
        const paramsShape = route.schema?.shape?.params;
