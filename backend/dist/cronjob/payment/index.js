 ...
 */
export function startGhnShipmentRetryJob() {
    cron.schedule(env.GHN_SHIPMENT_RETRY_CRON, async () => {
        try {
            ...
            if (result.scanned > 0) {
                ...
                if (stillPending > 0) {
                    await notificationService.notifyAdminSystemAlert("Vẫn còn đơn thiếu vận đơn GHN", `Có ${stillPending} đơn đã thanh toán nhưng vẫn chưa tạo được vận đơn GHN sau lượt retry tự động. Cần kiểm tra và tạo vận đơn thủ công nếu GHN tiếp tục lỗi.`);
                }
            }
        }
        catch (error) {
