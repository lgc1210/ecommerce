...
async function getServiceId(toDistrictId) {
...
export async function calculateShippingFee({ toDistrictId, toWardCode, weightGram, lengthCm, widthCm, heightCm, insuranceValue }) {
    try {
 ...
 *    vào status code.
 */
function isTransientGhnError(error) {
    if (!error?.response)
        return true;
 ...
 */
export async function createShippingOrder({ clientOrderCode, toName, toPhone, toAddress, toWardCode, toDistrictId, codAmount, weightGram, lengthCm, widthCm, heightCm, insuranceValue, items, }) {
    let lastError;
    ...
    for (let attempt = 1; attempt <= GHN_CREATE_ORDER_MAX_ATTEMPTS; attempt++) {
        try {
            ...
            if (!isNormalizedBusinessError && isTransientGhnError(error) && attempt < GHN_CREATE_ORDER_MAX_ATTEMPTS) {
                console.warn(`[ghn] Tạo vận đơn thất bại tạm thời (lần ${attempt}/${GHN_CREATE_ORDER_MAX_ATTEMPTS}) cho đơn ${clientOrderCode}, thử lại sau ${GHN_CREATE_ORDER_RETRY_DELAY_MS}ms:`, error?.response?.data?.message ?? error?.message ?? error);
 ...
 */
export async function cancelShippingOrder(ghnOrderCode) {
    try {
