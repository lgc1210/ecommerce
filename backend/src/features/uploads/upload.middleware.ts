...
export const WARRANTY_CLAIM_UPLOAD_DIR = path.join(UPLOAD_ROOT, "warranty-claims");

if (!fs.existsSync(PRODUCT_UPLOAD_DIR)) {
	fs.mkdirSync(PRODUCT_UPLOAD_DIR, { recursive: true });
}
if (!fs.existsSync(WARRANTY_CLAIM_UPLOAD_DIR)) {
	fs.mkdirSync(WARRANTY_CLAIM_UPLOAD_DIR, { recursive: true });
}
...
/** Factory dùng chung cho mọi endpoint chỉ nhận 1 file ảnh (field "image") — khác nhau duy nhất
 * ở thư mục lưu. Tách ra để không lặp lại y hệt cấu hình multer cho từng use-case (product-image,
 * warranty-claim-image...). */
function createSingleImageUploadMiddleware(destDir: string) {
	...
	// Bọc middleware của multer lại để trả lỗi theo đúng format JSON nhất quán với phần còn lại
	// của API (thay vì để lỗi rơi xuống default error handler của Express).
...
export const uploadProductImageMiddleware = createSingleImageUploadMiddleware(PRODUCT_UPLOAD_DIR);
export const uploadWarrantyClaimImageMiddleware = createSingleImageUploadMiddleware(WARRANTY_CLAIM_UPLOAD_DIR);
