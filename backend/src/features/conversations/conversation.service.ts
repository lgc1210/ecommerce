...
class ConversationService {
	...
	async getConversationOrThrow(id: number) {
	 ...
	 */
	async getOrCreateOwnConversation(customerId: number) {
	...
	async getOwnCurrentConversation(customerId: number) {
	...
	async getOwnConversation(customerId: number, conversationId: number) {
	...
	async listConversationsForStaff(query: Record<string, any>) {
	 ...
	 */
	async assignToSelf(conversationId: number, staffId: number) {
	...
	async updateStatus(conversationId: number, status: ConversationStatus) {
	...
	async markReadByCustomer(conversationId: number, customerId: number, lastReadMessageId: number) {
	...
	async markReadByStaff(conversationId: number, lastReadMessageId: number) {
		const conversation = await this.getConversationOrThrow(conversationId);
		await prisma.conversation.update({
			where: { id: conversationId },
